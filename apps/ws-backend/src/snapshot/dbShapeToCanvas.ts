...
const withRotation = (shape: DbShape) => (base: Record<string, unknown>) =>
  ({ ...base, rotation: shape.rotation ?? 0 });
...
export function mapDbShapeToCanvas(shape: DbShape): CanvasShape {
  ...
  if (type === "box" && shape.width != null && shape.height != null) {
    return { ...base, type: "box", width: shape.width, height: shape.height } as CanvasShape;
  }
  if (type === "ellipse" && shape.width != null && shape.height != null) {
    return { ...base, type: "ellipse", width: shape.width, height: shape.height } as CanvasShape;
  }
  if (type === "line" && shape.points != null && Array.isArray(shape.points)) {
    return { ...base, type: "line", points: shape.points as { x: number; y: number }[] } as CanvasShape;
  }
  if (type === "triangle" && shape.width != null && shape.height != null) {
    return { ...base, type: "triangle", width: shape.width, height: shape.height } as CanvasShape;
  }
